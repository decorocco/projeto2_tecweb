...
export default class About extends Component {
    ...
    onClickTranslate(e) {
        axios.post( "https://rapidapi.p.rapidapi.com/piglatin.json",
        {text: (this.state.text)
        }, {headers: {
            'x-rapidapi-key': '77d70301cbmsh13ce25fdaf06c15p11ad75jsnf8b429fd3c6c',
            'x-rapidapi-host': "piglatin.p.rapidapi.com"
        }})
        .then((response)=>{
            this.setState({
            ...
            axios.post("http://localhost:5000/history/add/", {
                language: "Pig Latin", input: this.state.text, translation: this.state.translation
            })
        })
        .catch((error)=> {
            console.log(error)
